...
## Java语言的环境搭建
  ...
  2. 简单而言，使用JDK的开发工具完成的java程序，交给JRE去运行。
     ...
     - JRE = JVM + Java SE标准类库
     ![java虚拟机](../../assent/javase/jdk1.png)

     ![java虚拟机](../../assent/javase/jdk2.png)
  ...
  4. **配置环境变量**
     ...
     - 在dos命令行中敲入javac，出现错误提示：
       ![jdk环境报错](../../assent/javase/jdkerror1.png)
     - 错误原因：当前执行的程序在当前目录下如果不存在，windows系统会在系统中已有的一个名为path的环境变量指定的目录中查找。如果仍未找到，会出现以上的错误提示。所以进入到jdk安装路径\bin目录下，执行javac，会看到javac参数提示信息。
       ![jdk环境报错](../../assent/javase/jdkerror2.png)
  ...
  5. **每次执行java 的工具都要进入到bin目录下，非常麻烦。可不可以在任何目录下都可以执行java的工具呢？**
     ...
     - 配置方法：
       ...
       ![jdk环境报错](../../assent/javase/jdkhandle1.png)

  6. **验证是否成功**
     ![jdk环境报错](../../assent/javase/jdksuccess1.png)

     ![jdk环境报错](../../assent/javase/jdksuccess2.png)

     ![jdk环境报错](../../assent/javase/jdksuccess3.png)
...
4. **通过java 命令对生成的class 文件进行运行。**
   ...
   - 通过运行工具java.exe对字节码文件进行执行。
     ![jdk环境报错](../../assent/javase/1-zhixing.png)

   - 出现提示：缺少一个名称为main的方法。
     ![jdk环境报错](../../assent/javase/1-zhixing2.png)
   ...
   - 想要和JVM来个互动，只要在main方法中加入一句System.out.println(“Hello World");因为程序进行改动，所以再重新编译，运行即可。
     ![jdk环境报错](../../assent/javase/1-bianyigoucheng.png)

## 常见问题及解决方法

1. **问题1**

   ![jdk环境报错](../../assent/javase/1-wenti1.png)
...
2. **问题2**
   ...
   ![jdk环境报错](../../assent/javase/1-wenti2.png)
...
3. **问题3**
   ...
   ![jdk环境报错](../../assent/javase/1-wenti3.png)

   - 声明为public的类应与文件名一致，否知编译失败

4. **问题4**

   ![jdk环境报错](../../assent/javase/1-wenti4.png)
   ...
   - **文档注释(java特有)**
     ...
     - **格式**：
       ...
       **注释内容可以被JDK提供的工具javadoc所解析，生成一套以网页文件形式体现的该程序的说明文档。**
       ...
       ![jdk环境报错](../../assent/javase/1-zhushi.png)
